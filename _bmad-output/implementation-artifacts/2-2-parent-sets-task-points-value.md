...
- [ ] Task 1: 扩展数据库表支持积分值 (AC: 积分值记录在tasks表)
  - [ ] 1.1 验证task_plans和tasks表已有points字段（integer, 1-100）
  - [ ] 1.2 如果不存在，创建迁移添加points字段
  - [ ] 1.3 更新lib/db/queries/task-plans.ts添加积分值查询支持
  - [ ] 1.4 更新lib/db/queries/tasks.ts添加积分值查询支持

- [ ] Task 2: 实现积分值输入组件 (AC: 积分值必须为正整数1-100)
  - [ ] 2.1 创建PointsInput组件（Shadcn Input + validation）
  - [ ] 2.2 实现输入验证（仅允许正整数1-100）
  - [ ] 2.3 实现实时错误提示（超出范围、非数字）
  - [ ] 2.4 添加"建议积分值"提示（基于任务类型）

- [ ] Task 3: 实现任务难度-积分值建议系统 (AC: 显示积分值与任务难度的建议对应关系)
  - [ ] 3.1 创建积分值建议常量（constants/points-suggestions.ts）
  - [ ] 3.2 实现任务类型到积分值范围的映射
  - [ ] 3.3 在TaskPlanForm中集成积分值建议UI
  - [ ] 3.4 实现积分值预选按钮（简单/中等/困难/特殊）

- [ ] Task 4: 集成积分值到任务模板创建流程 (AC: 在创建任务模板时设置积分值)
  - [ ] 4.1 扩展TaskPlanForm组件添加积分值字段
  - [ ] 4.2 实现积分值字段与任务类型的联动（自动填充建议值）
  - [ ] 4.3 更新API端点接受积分值参数
  - [ ] 4.4 验证积分值范围（1-100）后保存到数据库
...
- [ ] Task 6: 实现任务完成时积分结算逻辑 (AC: 任务完成并审批后，积分自动累加)
  - [ ] 6.1 创建积分结算服务（lib/services/points-calculator.ts）
  - [ ] 6.2 实现任务审批时的积分累加逻辑
  - [ ] 6.3 更新lib/db/queries/points.ts添加积分记录创建函数
  - [ ] 6.4 更新儿童积分余额（lib/db/queries/point-balances.ts）

- [ ] Task 7: 编写BDD测试 (AC: 所有验收条件)
  - [ ] 7.1 Given-When-Then格式：积分值设置验证测试
  - [ ] 7.2 测试积分值范围验证（1-100边界值测试）
  - [ ] 7.3 测试任务类型-积分值建议UI
  - [ ] 7.4 测试任务完成时积分结算逻辑
  - [ ] 7.5 测试积分自动累加到儿童账户

- [ ] Task 8: 实现用户反馈和错误处理 (AC: 用户体验要求)
  - [ ] 8.1 使用Shadcn Toast显示积分设置成功/失败
  - [ ] 8.2 处理无效积分值输入（实时错误提示）
...
- `lib/constants/points-suggestions.ts` - Points suggestion constants
- `components/forms/points-input.tsx` - Points input component
- `components/forms/points-suggestions.tsx` - Points suggestion UI
- `lib/services/points-calculator.ts` - Points calculation service
- `lib/db/queries/points.ts` - Points history queries
- `lib/db/queries/point-balances.ts` - Points balance queries
- `components/forms/task-plan-form.tsx` - Update to add points field
- `components/forms/one-time-task-form.tsx` - Create for one-time tasks (Story 2.12)
- `app/api/task-plans/route.ts` - Update to accept points parameter
- `tests/integration/points-setting.spec.ts` - Integration tests
- `tests/e2e/points-setting.spec.ts` - E2E tests
