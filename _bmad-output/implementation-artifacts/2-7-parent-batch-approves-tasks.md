...
- [ ] Task 1: 实现任务审批页面UI (AC: 进入任务审批页面，显示待审批任务列表)
  - [ ] 1.1 创建TaskApprovalList组件（Shadcn Table + Checkbox）
  - [ ] 1.2 实现任务卡片展示（任务名称、图标、完成时间）
  - [ ] 1.3 实现孩子姓名显示（已完成任务的孩子）
  - [ ] 1.4 实现任务完成证明展示（如有照片）
  - [ ] 1.5 实现任务状态筛选（待审批/已批准/已驳回）
  - [ ] 1.6 实现按儿童筛选（查看特定孩子的待审批任务）

- [ ] Task 2: 实现批量选择功能 (AC: 全选/取消全选)
  - [ ] 2.1 实现任务行复选框（Shadcn Checkbox）
  - [ ] 2.2 实现"全选"按钮（选择当前页所有任务）
  - [ ] 2.3 实现"取消全选"按钮
  - [ ] 2.4 实现滑动多选（触摸手势，如微信长按）
  - [ ] 2.5 实现长按全选（长按任意任务→全选当天任务）
  - [ ] 2.6 实现选中状态管理（Zustand store）

- [ ] Task 3: 实现批量通过功能 (AC: 批量通过：一次性审批所有选中任务)
  - [ ] 3.1 创建BatchApproveDialog组件（Shadcn Dialog）
  - [ ] 3.2 实现选中任务列表展示
  - [ ] 3.3 实现批量确认逻辑（确认通过数量）
  - [ ] 3.4 实现积分计算和累加（所有选中任务的积分总和）
  - [ ] 3.5 更新API端点支持批量通过
  - [ ] 3.6 实现批量通过后的任务状态更新

- [ ] Task 4: 实现批量驳回功能 (AC: 批量驳回：一次性驳回所有选中任务，需填写驳回原因)
  - [ ] 4.1 创建BatchRejectDialog组件（Shadcn Dialog + Textarea）
  - [ ] 4.2 实现驳回原因输入框（必填，最多200字）
  - [ ] 4.3 实现预设驳回原因选择（任务没有完成/完成质量不达标等）
  - [ ] 4.4 实现选中任务列表展示
  - [ ] 4.5 实现批量确认逻辑（确认驳回数量）
  - [ ] 4.6 更新API端点支持批量驳回
  - [ ] 4.7 实现批量驳回后的任务状态更新（返回待完成）

- [ ] Task 5: 实现单个任务审批功能 (AC: 审批按钮："通过"和"驳回")
  - [ ] 5.1 实现单个任务"通过"按钮
  - [ ] 5.2 实现单个任务"驳回"按钮
  - [ ] 5.3 实现单个任务驳回对话框（填写驳回原因）
  - [ ] 5.4 复用批量审批/驳回逻辑（单个任务视为批量=1）
  - [ ] 5.5 实现单个任务操作反馈

- [ ] Task 6: 实现积分累加和结算逻辑 (AC: 审批通过后，积分立即累加到儿童账户)
  - [ ] 6.1 扩展PointsCalculator服务（从Story 2.2）
  - [ ] 6.2 实现批量积分计算（所有通过任务的积分总和）
  - [ ] 6.3 实现积分余额更新（原子事务）
  - [ ] 6.4 实现积分历史记录创建（每个任务一条记录）
  - [ ] 6.5 实现积分更新通知推送（Story 2.14）

- [ ] Task 7: 实现操作结果显示 (AC: 批量操作后，显示操作结果)
  - [ ] 7.1 创建OperationResultToast组件（Shadcn Toast）
  - [ ] 7.2 实现操作结果展示（"已通过X个任务，驳回Y个任务"）
  - [ ] 7.3 实现积分变化展示（"+Z积分"）
  - [ ] 7.4 实现操作历史记录（审计日志）
  - [ ] 7.5 实现错误处理和重试机制

- [ ] Task 8: 实现任务完成证明展示 (AC: 任务完成证明（如有照片）)
  - [ ] 8.1 实现照片上传功能（任务完成时拍照或相册选择）
  - [ ] 8.2 实现照片缩略图展示（审批列表）
  - [ ] 8.3 实现照片全屏预览（点击缩略图查看大图）
  - [ ] 8.4 实现照片存储（本地或图床）
  - [ ] 8.5 实现无照片时的默认图标展示

- [ ] Task 9: 编写BDD测试 (AC: 所有验收条件)
  - [ ] 9.1 Given-When-Then格式：批量审批集成测试
  - [ ] 9.2 测试全选/取消全选功能
  - [ ] 9.3 测试批量通过功能（多个任务）
  - [ ] 9.4 测试批量驳回功能（多个任务，驳回原因）
  - [ ] 9.5 测试单个任务审批功能
  - [ ] 9.6 测试积分累加和结算逻辑
  - [ ] 9.7 测试操作结果显示
  - [ ] 9.8 性能测试（批量操作<500ms，NFR3）

- [ ] Task 10: 实现错误处理和用户反馈 (AC: 用户体验要求)
  - [ ] 10.1 使用Shadcn Toast显示操作成功/失败
  - [ ] 10.2 处理网络错误（离线状态）
  - [ ] 10.3 处理权限错误（非家长用户）
  - [ ] 10.4 处理任务状态冲突（任务已被其他家长审批）
  - [ ] 10.5 实现操作确认对话框（防止误操作）
...
- `database/schema/tasks.ts` - Update with proof_image, rejectionReason, approvedBy, approvedAt fields
- `database/migrations/xxx_add_approval_fields_to_tasks.sql` - Migration file
- `lib/services/points-calculator.ts` - Extend with batch approval/rejection methods
- `lib/store/approval-store.ts` - Zustand store for selection state
- `components/features/task-approval-list.tsx` - Task approval list component
...
- `components/ui/operation-result-toast.tsx` - Operation result toast
- `components/features/proof-image-preview.tsx` - Proof image preview
- `app/api/tasks/batch-approve/route.ts` - Batch approve API endpoint
- `app/api/tasks/batch-reject/route.ts` - Batch reject API endpoint
- `tests/integration/batch-approval.spec.ts` - Integration tests
- `tests/unit/points-calculator-batch.spec.ts` - Unit tests for batch approval
- `tests/e2e/batch-approval.spec.ts` - E2E tests
