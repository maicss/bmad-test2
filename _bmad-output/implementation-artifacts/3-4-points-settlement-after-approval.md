...
**Story 3.9 Responsibility (Base Infrastructure):**
- **Story 3.9 creates**: `points_history` and `point_balances` table schemas
- **Story 3.9 implements**: Base atomic transaction functions for points operations
- **Story 3.9 provides**: Core transaction utility functions (addPoints, deductPoints, getBalance)
- **This story (3.4) extends**: Story 3.9's base transaction functions with approval check logic

**Clear Responsibility Boundary:**
```typescript
// Story 3.9 provides (lib/services/points-calculator.ts):
export async function addPointsToChild(
  ...
  // Core atomic transaction implementation
  // - Updates point_balances.balance
  // - Inserts into points_history
  // - Uses db.transaction() for atomicity
  // - This function does NOT check task approval status
}

// This story (3.4) extends (app/api/tasks/[id]/approve/route.ts):
export async function POST(req: NextRequest, { params }) {
  // Parent approval workflow
  const task = await getTask(params.id);

  // Approval check logic (THIS STORY)
