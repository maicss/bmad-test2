...
**ADR-3 决策：**
- **选择：** Better-Auth 1.4.18+ with phone plugin AND password plugin + PIN码登录
- **理由：**
  1. 项目上下文强制选择
  2. Better-Auth处理安全最佳实践
  3. **家长支持双认证方式：手机号 + OTP（短信验证）和手机号 + 密码**
  4. PIN码满足儿童登录需求
  5. 36小时会话滚动刷新符合PRD要求
  6. 双认证方式提升用户体验（无网络时可用密码登录）
- **记录人：** Winston
- **日期：** 2026-02-12
- **更新日期：** 2026-02-13（添加密码支持）
- **状态：** 已采纳（已更新）
- **替代方案：** OAuth 2.0 + JWT
- **后果：**
  - 儿童端需要手动输入PIN码；不支持生物识别登录
  - **新增：家长端需要配置双认证插件，增加配置复杂度但提升灵活性**
