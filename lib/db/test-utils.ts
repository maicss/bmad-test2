...
export async function createChild(options: { familyId: string }) {
  ...
  const result = await db.insert(users).values({
    id,
    phone: `child_${id.slice(0, 8)}@test.local`,
    phone_hash: await Bun.password.hash(`child_${id.slice(0, 8)}`, 'bcrypt'),
    password_hash: pinHash,
    role: 'child',
    family_id: options.familyId,
    name: `Child-${id.slice(0, 8)}`,
  }).returning() as any;
